...
contract FreelancerNFT is ERC721URIStorage, Ownable {
    ...
    enum UserType { Freelancer, Company }
    ...
    event NFTMinted(address indexed user, uint256 indexed tokenId, UserType userType);
...
function updateMetadataURI(uint256 tokenId, string memory newURI) external onlyTokenOwner(tokenId) {
...
function mintUserNFT(UserType _userType, string memory _tokenURI) public returns (uint256) {
